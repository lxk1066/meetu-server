...
// 发布帖子，回滚所有已保存的图片
const rollBackPictures = successPictures => {
...
class Square {
  ...
  async publishPost(ctx) {
    ...
    if (!body) {
      ...
      if (Reflect.get(files, "pictures")) {
        ...
        if (pictures.length > MaxPictures) {
          ...
          for (const picture of pictures) {
            ...
            if (result.status === "Done") successPictures.push({ filename, size: result.size, createdTime });
            else {
        ...
        if (pictures.length !== successPictures.length) {
          rollBackPictures(successPictures);
      ...
      `)
        .then(async result => {
          const insertId = result.insertId; // 帖子ID
          ...
          if (successPictures.length) {
            ...
            successPictures.forEach(picture => {
              sqls.push(`insert into meetu_square_pictures(pic_name, size, art_id, updated_time) 
                     values("${picture.filename}",${picture.size},${insertId},"${picture.createdTime}");`);
            });
            await transaction(sqls)
              .then(() => {
                ctx.body = { code: 200, msg: "发布成功", data: { postId: insertId } };
              })
              .catch(() => {
                rollBackPictures(successPictures);
                ctx.body = { code: 500, msg: "图片上传失败" };
              });
          } else {
            ctx.body = { code: 200, msg: "发布成功", data: { postId: insertId } };
          }
