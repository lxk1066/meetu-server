    ...
    if (res1.length === 0 && res2.length === 0) {
      ...
      await queryDB(`INSERT INTO meetu_users 
            ...
            .then(async ({ insertId }) => {
              ...
              await queryDB(`INSERT INTO meetu_users_muid VALUES ("${insertId}_${randomStr(10-insertId.toString().length-1)}", "${insertId}", "0")`)
                    .then(() => {
...
const getMuidUserInfo = async ctx => {
  ...
  if (!muid) {
    ...
    await queryDB(`select user_id from meetu_users_muid where muid="${muid}"`).then(async res => {
